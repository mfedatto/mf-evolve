...
public interface IMigrationDefinitions : IPlaceholder
{
	...
	/// <summary>
	///     An <see cref="IConnectionStringTemplate" /> for the database
	///     connection string.
	/// </summary>
	IConnectionStringTemplate? ConnectionStringTemplate { get; }

	/// <summary>
	///     An <see cref="IWorkingDirectoryTemplate" /> for the Evolve execution
	///     working directory.
	/// </summary>
	IWorkingDirectoryTemplate? WorkingDirectoryTemplate { get; }

	/// <summary>
	///     Paths to scan recursively for migrations. Mandatory if
	///     <see cref="EmbeddedResourceAssemblies" /> is empty.
	/// </summary>
	string[]? Locations { get; }

	/// <summary>
	///     A list of schemas managed by Evolve. If empty, the default schema
	///     for the datasource connection is used.
	/// </summary>
	string[]? Schemas { get; }
	...
	/// <summary>
	///     Transaction mode settings for the migration. See
	///     <see cref="TransactionModes" /> for possible values.
	/// </summary>
	...
	/// <summary>
	///     When <c>true</c>, if validation phase fails, Evolve will erase the
	///     database schemas and re-execute migration scripts from scratch.
	...
	/// <summary>
	///     Execute repeatedly all repeatable migrations for as long as the
	///     number of errors decreases.
	/// </summary>
	bool? RetryRepeatableMigrationsUntilNoError { get; }

	/// <summary>
	///     Assemblies to scan to load embedded migration scripts. Mandatory if
	///     <see cref="Locations" /> is empty.
	/// </summary>
	string[]? EmbeddedResourceAssemblies { get; }

	/// <summary>
	///     Includes embedded migration scripts that start with one of these
	///     filters.
	/// </summary>
	string[]? EmbeddedResourceFilters { get; }

	/// <summary>
	///     The time in seconds to wait for the migration to execute before
	///     generating an error.
	/// </summary>
	...
	/// <summary>
	///     The schema containing the metadata table. If not set, defaults to
	///     the first schema defined in Schemas or the one of the datasource
	///     connection.
	/// </summary>
	...
	/// <summary>
	///     When <c>true</c>, Evolve will use a session level lock to coordinate
	///     migrations on multiple nodes.
	/// </summary>
